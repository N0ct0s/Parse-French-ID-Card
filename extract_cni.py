...
import cv2
import numpy
def getSimilarity(kp_1, desc_1, template,orb,flaan):
    kp_2, desc_2 = orb.detectAndCompute(template, None)
    matches = flaan.knnMatch(desc_1, desc_2, k=2)
    good_points = []
    for m_n in matches:
        if len(m_n) != 2:
            continue
    ...
    #Not beautiful to return an array with a lot of things but it means you don't have to redo FLAAN's knnMatch and ORB's detectAndCompute
    return [len(good_points)/number_keypoints, good_points, [kp_1, kp_2]]

def isNew(image, orb, flaan):
  ...
  similarity = getSimilarity(kp_1, desc_1, templatenew, orb,flaan)
  if (similarity[0]*100) > 1.1:
     print("isNew runned during "+str(int(round(time.time() * 1000))-start))
     return [True,similarity]
  templateold = cv2.imread("template-old-cni.jpg")
  similarity = getSimilarity(kp_1, desc_1, templateold, orb,flaan)
  if (similarity[0]*100) > 1.1:
      print("isNew runned during "+str(int(round(time.time() * 1000))-start))
...
def getCNI(tobealigned):
    ...
    reference = None
    orb_detector = cv2.ORB_create(4760)
    flaan = cv2.FlannBasedMatcher(dict(algorithm = 6, table_number = 6,  key_size = 12,  multi_probe_level = 1), {})
    isCNINew = isNew(tobealigned, orb_detector, flaan)
    match(isCNINew[0]):
        case True:
    ...
    return [transformed_img, isCNINew]
cv2.imshow("mat",getCNI(cv2.imread("test-new.jpg"))[0])
cv2.waitKey(0)